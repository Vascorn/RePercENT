...
# Mixture of Experts FeedForward layer, where each expert is a standard FFN and the router determines which experts to use for each token.
...
# Standard multi-head attention, used for latent self-attention layers and optionally for cross attention layers if not using Slot Attention.
...
# Modified attention mechanism for cross attention layers, implementing Slot Attention-style competitive attention where the softmax is taken over the queries (slots) rather than the keys, 
# and optionally within groups of queries for conditional competition. This can encourage specialization among slots and is particularly beneficial for certain data types like images.
...
# main class
# The main Perceiver class. The core functionality, including the iterative cross and self attention blocks is based on the implementation from https://github.com/lucidrains/perceiver-pytorch/tree/main. 
# However, as we introduce group slot attention and MoE FFN modifications, we have made significant changes to the architecture and forward pass, so we implement our own version here.
class Perceiver(nn.Module):
    def __init__(
        ...
        *,
        ...
        NOTE: The original Perceiver architecture uses standard Fourier positional encoding for the input data. However, as we work on pre-extracted embeddings, the position is already encoded in the input data, 
        so we recommend setting fourier_encode_data to False, as they are usually redundant and can create unnecessary computational overhead.
        """
        ...
        # use gram schmidt process to initialize latents to be orthogonal, which can help with optimization and specialization of latents.
        with torch.no_grad():
            eps = 1e-6
        ...
        if not use_moeffn:
            ...
            # Alternative FF architecture, not used in the paper.
            # Modification of the Feed-forward layers in the latent blocks to be Mixture of Experts, using one expert per latent vector (num_experts = num_latents) to encourage specialization, 
            # with a schedule for top_k and gate dropout to encourage expert specialization in later layers.
        ...
        # Building the layers of the Perceiver.
        ...
        # classifier (optional) head that mean pools the latents and projects to number of classes (linear probing).
        # This follows the original Perceiver design, however for the paper we take the complete set of latents.  
